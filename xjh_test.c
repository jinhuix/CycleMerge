...
int GetMissingNumbers(uint32_t N, uint32_t *A, uint32_t size) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        if (A[i] >= 1 && A[i] <= N) {
            if (flags[A[i]] == 1) {
                printf("Error! IO number %u is duplicated.\n", A[i]);
            }
            flags[A[i]] = 1;
        }
    }
...
void SaveKeyMetricsToFile(const char *filename, const KeyMetrics *metrics, char algorithm_name[]) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < numAlgorithms; i++) {
        ...
        if (metrics.errorIOCount > 0) {
            metrics.algorithmRunningDuration = 0;   // 总微秒数
            metrics.addressingDuration = 0;         // 访问时间
            metrics.tapeBeltWear = 0;               // 带体磨损
            metrics.tapeMotorWear = 0;              // 电机磨损
        } else {
            AccessTime accessTime = {0};                                                        
            TotalAccessTime(inputParam, output, &accessTime);                                   // 访问时间
            metrics.addressingDuration = accessTime.addressDuration;                            // 寻址时长
            metrics.readDuration = accessTime.readDuration;                                     // 读取时长
            TapeBeltSegWearInfo segWearInfo = {0};                                              // 带体磨损
            ...
            metrics.tapeMotorWear = TotalMotorWearTimes(inputParam, output);                     // 电机磨损
        }

        metrics.algorithmRunningDuration = ((seconds) * 1000000 + useconds) / 1000.0;            // 算法运行时长
        metrics.memoryUse = 0;                                                                   // 内存占用
